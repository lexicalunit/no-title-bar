...
function getTabBarHeight () {
  ...
  return tabBar ? window.getComputedStyle(tabBar).height : '0'
}

function panelMarginTop () {
  let lhs = parseFloat(tabBarHeight.replace('px', ''))
  let rhs = parseFloat(titleBarHeight.replace('px', ''))
  return Math.max(lhs, rhs) + 'px'
}

function setPanelMargins () {
  let marginTop = fullscreen
    ? '0'
    : panelMarginTop()
